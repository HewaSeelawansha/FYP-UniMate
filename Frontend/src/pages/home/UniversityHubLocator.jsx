...
const UniversityHubLocator = () => {
  ...
  const [selectedUniversity, setSelectedUniversity] = useState('NSBM');
  const [mapCenter, setMapCenter] = useState([6.821380, 80.041691]); // Default: NSBM
  const [filterDistance, setFilterDistance] = useState(2); // km
  const [filteredBoardings, setFilteredBoardings] = useState([]);

  // Universities data
  const universities = [
    { id: 'NSBM', name: 'NSBM Green University', location: [6.821380, 80.041691] },
    { id: 'UoC', name: 'University of Colombo', location: [6.900500, 79.858600] },
    { id: 'UoP', name: 'University of Peradeniya', location: [7.254500, 80.597800] }
  ];
  ...
  useEffect(() => {
    if (boardings.length > 0) {
      const filtered = boardings.filter(boarding => {
        ...
        const lat1 = university.location[0];
        const lon1 = university.location[1];
  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        >
          <p className="text-xl font-semibold uppercase text-green-500 mb-2">Interactive Map</p>
          <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">
            Find Boarding Houses Near Your University
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Explore available accommodations around your campus and find the perfect place to call home during your studies.
          </p>
        </motion.div>
...
>
  ...
  <div className="flex-1 min-w-[200px] flex flex-col justify-between">
    <label className="block text-sm font-medium text-gray-700 mb-1">Distance ({filterDistance} kilometers)</label>
    <div className="relative h-full">
      <input
        type="range"
        min="1"
  ...
  <div className="flex-1 min-w-[200px] flex flex-col justify-between">
    ...
    <div className="bg-green-50 border border-green-500 p-2 rounded-lg text-center h-full flex items-center justify-center">
      <span className="font-semibold text-green-500">
        {boardingLoading ? 'Loading...' : `${filteredBoardings.length} properties found`}
      </span>
    </div>
  </div>
</motion.div>
          ...
          {boardingLoading ? (
            ...
            >
              ...
              {filteredBoardings.map((boarding, index) => (
                ...
                >
                  <Popup className="custom-popup">
                    <div className="font-semibold text-gray-800">{boarding.name}</div>
                    <div className="text-sm text-gray-600">Owner: {boarding.owner}</div>
                    {boarding.price && (
                      <div className="mt-1 font-medium text-green-600">${boarding.price}/month</div>
                    )}
                  </Popup>
                </Marker>
              ))}
